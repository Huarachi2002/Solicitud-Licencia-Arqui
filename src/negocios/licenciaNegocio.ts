...
import DLicenciaDetalle from "../datos/licenciaDetalleDato"
...
class NLicencia {
  ...
  async solicitarLicenciaEstudiante(data: {
    ...
    const licencia = await DLicencia.solicitarLicenciaEstudiante(data);

    await DLicenciaDetalle.guardarDetalle(licencia.id, data.ids_grupo)
